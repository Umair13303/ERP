...
/* ------ MPO List Operation ------ */
var supplierSummaryTable = "";
function domsupplierSummaryTable() {
    supplierSummaryTable = $('#TableSupplierSummary').DataTable({
        "processing": true,
        "serverSide": false,
        "responsive": true,
        "ordering": true,
        ...
        "ajax": {
            ...
            "error": function (xhr, error, thrown) {
                toastr.error("Failed to load supplier summary.");
            }
        },
        "oLanguage": {
            "oPaginate": {
        ...
        "columns": [
            ...
            { "data": "description", "title": "supplier" },
            ...
            { "data": "supplierId", "title": "supplierId", visible: false },
        ],
        columnDefs: [
        ],
    });
    supplierSummaryTable.on('order.dt search.dt draw.dt', function () {
        supplierSummaryTable.column(0, { search: 'applied', order: 'applied' })
...
function createUpdateDataIntoDB() {
    ...
    var defaultPayableAccount = null;
    if (isAutoChartOfAccount == true) {
        defaultPayableAccount = description.trim() + " Default Payable Account";
    }
